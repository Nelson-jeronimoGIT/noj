...
crud_cliente.cud = (req,res)=>{
     ...
     const fecha_nacimiento = req.body.txt_fecha_fn;
     if(btn_crear){
        conectar.query('insert into clientes set?',{nit:nit,nombres,apelliodos:apelliodos,direccion:direccion,telefono:telefono,fecha_nacimiento:fecha_nacimiento},(error,results)=>{ });
        if (error){
            console.log(error);
        }else{
            res.redirect('/');
     ...
     if(btn_actualizar){
        conectar.query('update  clientes  where id_cliente =?',[{nit:nit,nombres,apelliodos:apelliodos,direccion:direccion,telefono:telefono,fecha_nacimiento:fecha_nacimiento},id_cliente],(error,results)=>{ });
        if (error){
            console.log(error);
        }else{
            res.redirect('/');
